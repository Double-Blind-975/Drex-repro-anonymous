...
improvements_alg_drexlb = {} # For alg_drexlb.csv vs others
...
for folder in os.listdir(base_dir):
    ...
    if os.path.isdir(folder_path):
        ...
        if match:
            ...
            # Compute differences using alg_drexlb.csv as the reference
            if 'alg_drexlb.csv' in csv_files:
                ref_df = csv_files['alg_drexlb.csv']
                diffs_drexlb = {}
                for file_name, df_other in csv_files.items():
                    if file_name != 'alg_drexlb.csv':
                        ...
                        avg_diff = diff.mean()
                        diffs_drexlb[file_name] = avg_diff
                        print(f"[{folder}] alg_drexlb.csv vs {file_name}: Avg throughput diff = {avg_diff:.3f}")
                improvements_alg_drexlb[folder] = diffs_drexlb
            else:
                print(f"Reference file alg_drexlb.csv not found in {folder_path}")
...
# Create figure for improvements using alg_drexlb.csv as reference
# ~ create_improvement_figure(improvements_alg_drexlb, "alg_drexlb.csv", os.path.join("plot", "combined", f"alg_drexlb_improvement_all{folder_suffix}.pdf"))

# Create the combined one
create_combined_improvement_figure(improvements_alg4, improvements_alg_drexlb, "plot/combined/combined_throughput_different_set_of_nodes_improvements_drex.pdf")
...
improvements_lb_datasets = {}  # For D-Rex LB (alg_drexlb.csv as reference)
...
for folder in folder_list:
    ...
    # Compute improvements for D-Rex LB using "alg_drexlb.csv" as the reference.
    if 'alg_drexlb.csv' in csv_files:
        ref_df = csv_files['alg_drexlb.csv']
        diffs_lb = {}
        for file_name, df_other in csv_files.items():
        ...
        print(f"Reference file alg_drexlb.csv not found in folder: {folder}")
